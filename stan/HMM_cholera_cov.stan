...
data {
  ...
  // Covariates
  // Ordered by 1) household, 2) participant
...
parameters {
  ...
  real logit_gamma; // recovery rate
...
transformed parameters {
  ...
  vector<lower=0, upper = 1>[sum(hh_size)] eh_prob;
  vector<lower=0, upper = 1>[sum(hh_size)] ih_prob_sym;
  vector<lower=0, upper = 1>[sum(hh_size)] ih_prob_asym;
  real gamma;
  real p_symp;
  real sigma;
  
  eh_prob = inv_logit(x_eh*beta_eh);
  ih_prob_sym = inv_logit((x_ih*beta_ih));
  ih_prob_asym = inv_logit((x_ih*beta_ih) + beta_asym);
  ...
  for(h in 1:n_hh) { // loop through household
    ...
    for (tt in 2:n_days) {
      ...
      no_hh_inf = to_vector(alpha[asym_rows, tt-1]).*rep_vector(1-ih_prob_asym[last_lik+p], hh_size[h]) +
                  to_vector(alpha[sym_rows, tt-1]).*rep_vector(1-ih_prob_sym[last_lik+p], hh_size[h]) + 
      ...
      no_inf = (1-eh_prob[last_lik+p])*prod(no_hh_inf); // Pr avoiding all infections
        ...
        logalpha[5*last_lik+5*(p-1)+2, tt] = log(exp(logalpha[5*last_lik+5*(p-1)+1, tt-1])*(prop_under1*(1-p_symp)*(1-no_inf)-epsilon/4) + // Pr (S>>Ia)
                                      exp(logalpha[5*last_lik+5*(p-1)+2, tt-1])*(1-gamma-3*epsilon/2) + // Pr (Ia>>Ia)
        ...
        logalpha[5*last_lik+5*(p-1)+3, tt] = log(exp(logalpha[5*last_lik+5*(p-1)+1, tt-1])*(prop_under1*p_symp*(1-no_inf)-epsilon/4) + // Pr (S>>Is)
                                      ...
                                      exp(logalpha[5*last_lik+5*(p-1)+3, tt-1])*(1-gamma-3*epsilon/2) + // Pr (Is>>Is)
        ...
        logalpha[5*last_lik+5*(p-1)+4, tt] = log(exp(logalpha[5*last_lik+5*(p-1)+1, tt-1])*epsilon + // Pr (S>>R)
                                      exp(logalpha[5*last_lik+5*(p-1)+2, tt-1])*(gamma-3*epsilon/2) + // Pr (Ia>>R)
                                      exp(logalpha[5*last_lik+5*(p-1)+3, tt-1])*(gamma-3*epsilon/2) + // Pr (Is>>R)
...
model {
  ...
  // beta coefficients
  beta_eh[1] ~ normal(-3,3); // EH intercept
  beta_eh[2:k_eh] ~ normal(0, 1); // EH coefficients
  beta_ih[1] ~ normal(-3,3); // IH intercept
  beta_ih[2:k_ih] ~ normal(0, 1); // IH coefficients
  beta_asym ~ normal(0,1);
  init_probs_hh ~  dirichlet(rep_vector(0.5, 5));
  logit_sigma ~ normal(0.9, 0.5);
  logit_gamma ~ normal(0, 0.5);
